...
import { TaxDocument } from '@/types/sharing';

interface LinkTaxItemsSheetProps {
...
const LinkTaxItemsSheet = ({ taxDocument, onClose }: LinkTaxItemsSheetProps) => {
  ...
  const linkedBillId = useMemo(
    () => DocumentLinkService.getLinkedBillIdForTax(taxDocument.id),
    [taxDocument.id, version]
  );
  ...
  const relatedIds = useMemo(
    () => DocumentLinkService.getRelatedDocumentIdsForTax(taxDocument.id),
    [taxDocument.id, version]
  );
  ...
  return (
    ...
    >
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Link this tax entry to a bill or document"
        initial={{ y: 100 }}
        animate={{ y: 0 }}
        exit={{ y: 100 }}
        ...
        <div className="flex items-start justify-between mb-5 gap-3">
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground">
              Connect “{taxDocument.name}” to the bill or document it came from. Linking doesn't
...
export default LinkTaxItemsSheet;
